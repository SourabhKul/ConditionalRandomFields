...
class CRF(object):
    ...
    def predict_logprob(self, X, W_F=None, W_T=None):
       ...
       Returns:
        ...
        #print('log_probs', np.sum(log_prob[1]))
        ...
        for i in range(N):
            ...
            for t in range(len(Y_i)):
                ...
                for c in range(C):
                    if Y_i[t] ==  Lol[c]:
                        gW_F[c] += F
                    gW_F[c] -= np.dot(F,np.exp(log_prob[t,c]))
    ...
    def fit(self, Y, X):
        ...
        print(scipy.optimize.check_grad(self.ll_wrapper,self.gll_wrapper,reshaped_parameters,Y,X))
